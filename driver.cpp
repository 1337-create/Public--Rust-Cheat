	...
	uint64_t& out_result)
{
	...
	if (result < sizeof(PacketHeader) ||
		completion_packet.header.magic != settingsI::packet_magic ||
	...
	const size_t	size)
{
	...
	packet.header.magic = settingsI::packet_magic;
...
SOCKET driver::connect()
{
	...
	address.sin_addr.s_addr = htonl(settingsI::server_ip);
	address.sin_port = htons(settingsI::server_port);
